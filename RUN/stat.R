...
plot(x=edge, y=inner, cex=0.3, xlab=edge_name, ylab=inner_name, las=1, bty="l", pch=".", text(x=min(edge, na.rm=T), y=max(inner, na.rm=T)*0.75, adj=0, labels=c(paste("pearson = ", Cor.P, "\n\n", sep=""), paste("spearman = ", Cor.S, sep=""))))
dev.off()
