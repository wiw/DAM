...
#   SAM_IN is supplied as an argument to this script.  The GATC fragments are
#   in a gff file in
...
###############################################################################
    ...
    if [ $# -lt 1 ]; then
      echo 'usage: reads2GATC.sh mappedReads.bam'
      echo 'aborting'
      exit 1
    elif [ $# -eq 2 ]; then
      SAM_VECTOR="$1 $2"
...
cat out.sam >> "${TMP_SAM}"
samtools view -bS "${TMP_SAM}" -o "${SAM_OUT}"
rm -f "${TMP_SAM}"
rm -f out.sam
